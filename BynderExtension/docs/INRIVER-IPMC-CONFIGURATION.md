...
| INITIAL_ASSET_LOAD_URL_QUERY | type=image,SyncToInriver=true | Filter query for the initial asset loader. Add query parts comma separated. You can find the query options on https://api.bynder.com/reference/get_api-v4-media. Custom meta properties don't need the property_ prefix.
| INITIAL_ASSET_LOAD_LIMIT | 10 | Limit of how many assets you want to import per run. 0 = unlimited|
...
| INRIVER_INTEGRATION_ID | 41a92562-bfd9-4847-a34d-4320bcef5e4a | This is a fixed id by Bynder. This probably won't change. See all the integration ids on the following URL: https://support.bynder.com/hc/en-us/articles/360013933619-How-To-Use-The-Bynder-Asset-Tracker
...
| BYNDER_BRAND_NAME | Customer Brand Name | Used to set the Brand in the upload of Assets. Can be found under Brand Management in Bynder (https://[CUSTOMER].getbynder.com/brands/management/) or with the API by running the GetAvailableBranches() method on the BynderClient. |
...
| TIMESTAMP_SETTINGS | {"dateTimeKind": "Local","localTimeZone": "W. Europe Standard Time","localDstEnabled": true} | Settings to use when setting a timestamp on archive events. DateTimeKind can be 'Utc' or 'Local'. DstEnabled should be true if your timezone uses Daylight Saving Time. Timezone id's can be found here 'http://www.xiirus.net/articles/article-_net-convert-datetime-from-one-timezone-to-another-7e44y.aspx' |
